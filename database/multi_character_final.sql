...
BEGIN
  ...
  IF NEW.speaker_type = 'character' AND NEW.speaker_character_id IS NOT NULL THEN
    ...
    EXCEPTION WHEN OTHERS THEN
      -- 如果转换失败，记录但不中断
      RAISE WARNING 'Failed to convert session_id % to UUID: %', NEW.session_id, SQLERRM;
    END;
  END IF;
...
-- 9. 为现有会话创建session_characters记录（安全方式）
DO $$
DECLARE
  session_rec RECORD;
